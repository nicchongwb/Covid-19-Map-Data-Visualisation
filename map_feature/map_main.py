...
# Some cleaning up of values in DF columns
def removeChar (value, char):
    return value.replace(char, "")

for key, value in df["caseNo"].items():
    df["caseNo"][key] = removeChar (value, "#")

for key, value in df["relatedCaseNo"].items():
    df["relatedCaseNo"][key] = removeChar (value, "#")
